...
OUTPUT_ROIS_FOLDER = 'output_rois' # Folder to save numbered 256x256 ROIs
OUTPUT_MASK_FOLDER = 'output_defects' # Folder to save macro binary masks
CALIB_JSON_PATH = 'calibration_metrics.json'

LABELS_CSV_PATH = 'roi_labels.csv'
SUMMARY_CSV_PATH = 'frame_roi_summary.csv'

os.makedirs(OUTPUT_ROIS_FOLDER, exist_ok=True)
...
def run_layer1_detector(img_path):
    ...
    output_path = os.path.join(OUTPUT_MASK_FOLDER, filename)
...
# =====================================================================
# STAGE 2: ROI MIDDLEWARE GENERATOR
# =====================================================================
def generate_rois(components):
    tagged_boxes = []

    # PRIORITY 1: Large Single Defects (Area > 700px)
    ...
    # PRIORITY 2: BFS Clusters (270px <= Area <= 700px, Distance < 120px)
    p2_pool = [c for c in components if 270 <= c["area"] <= 700 and not c["covered"]]
    
    if p2_pool:
        clusters = _bfs_spatial_clustering(p2_pool, max_dist=120.0, max_span=200)
    ...
    # PRIORITY 3: Near-Miss & Remaining Isolated Points
    ...
    for comp in p3_pool:
        ...
        for idx, item in enumerate(tagged_boxes):
            ...
            if dist < 120.0 and span_w <= 200 and span_h <= 200:
                tagged_boxes[idx] = (new_x1, new_y1, span_w, span_h, prio, num)
                comp["covered"] = True
                merged = True
                break

            elif 120.0 <= dist <= 180.0 and span_w <= 200 and span_h <= 200:
                tagged_boxes[idx] = (new_x1, new_y1, span_w, span_h, prio, num)
                comp["covered"] = True
                merged = True
                break
...
def _bfs_spatial_clustering(components, max_dist=120.0, max_span=200):
    n = len(components)
    visited = [False] * n
    clusters = []
...
# =====================================================================
# STAGE 3: CROP, INTERACTIVE SIDE-BY-SIDE LABELING & SAVE ENGINE
# =====================================================================
def extract_display_and_save_rois(img_path, tagged_boxes, start_counter, labels_writer):
    raw_rgb = cv2.imread(img_path, cv2.IMREAD_COLOR)
    if raw_rgb is None:
        return 0, start_counter

    img_h, img_w = raw_rgb.shape[:2]

    # 1. Centroid-Distance NMS (< 128px center distance suppression)
    deduped_boxes = []
    ...
    roi_count_this_frame = 0
    curr_counter = start_counter

    # 2. Extract Patches along with their Raw Coordinate Bounds
    patches_with_coords = []
    ...
    for item in deduped_boxes:
        ...
        if bw <= 256 and bh <= 256:
            ...
            if crop_rgb.shape[0] != 256 or crop_rgb.shape[1] != 256:
                crop_rgb = cv2.resize(crop_rgb, (256, 256), interpolation=cv2.INTER_AREA)

            patches_with_coords.append((crop_rgb, (x1, y1, x2, y2)))
            ...
            for ty in range(by, by + bh, stride):
                for tx in range(bx, bx + bw, stride):
                    ...
                    if (x2 - x1) < 256 and x2 == img_w:
                        x1 = max(0, img_w - 256)
                    if (y2 - y1) < 256 and y2 == img_h:
                        y1 = max(0, img_h - 256)
    ...
    # 3. Interactive Side-by-Side Display
    cv2.namedWindow("ROI Inspection (Press Y=Yes, N=No)", cv2.WINDOW_AUTOSIZE)

    for patch, (x1, y1, x2, y2) in patches_with_coords:
        roi_filename = f"{curr_counter}.png"
        
        # A. Full Frame View with Red Bounding Box
        full_vis = raw_rgb.copy()
        cv2.rectangle(full_vis, (x1, y1), (x2, y2), (0, 0, 255), 6)  # Red Box (Thickness 6)

        # Downscale Full Frame to 512px height preserving aspect ratio
        target_h = 512
        target_w = int(512 * (img_w / float(img_h)))
        full_vis_resized = cv2.resize(full_vis, (target_w, target_h))

        # B. Upscale ROI Patch to 512x512
        patch_resized = cv2.resize(patch, (512, 512), interpolation=cv2.INTER_NEAREST)

        # C. Add Labels to Visuals
        cv2.putText(full_vis_resized, "Full Frame (Red Box)", (15, 35), 
                    cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
        cv2.putText(patch_resized, f"ROI: {roi_filename} | Y/N?", (15, 35), 
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)

        # D. Combine Side-by-Side
        combined_display = np.hstack([full_vis_resized, patch_resized])
        
        cv2.imshow("ROI Inspection (Press Y=Yes, N=No)", combined_display)

        # Wait indefinitely for keypress (Y or N)
        label_str = ""
        while True:
        ...
        # Save ROI image & record label
        save_path = os.path.join(OUTPUT_ROIS_FOLDER, roi_filename)
        cv2.imwrite(save_path, patch)

        labels_writer.writerow([roi_filename, label_str])

        roi_count_this_frame += 1
        curr_counter += 1

    cv2.destroyAllWindows()
    return roi_count_this_frame, curr_counter
...
if __name__ == "__main__":
    ...
    print(f"Processing {len(image_paths)} images through Layer 1 & ROI Middleware...\n")

    # Global counter across all frames for ROI image naming (1.png, 2.png, ...)
    global_roi_counter = 1

    # Open CSV files for writing
    with open(LABELS_CSV_PATH, mode='w', newline='') as f_labels, \
         open(SUMMARY_CSV_PATH, mode='w', newline='') as f_summary:

        labels_writer = csv.writer(f_labels)
        summary_writer = csv.writer(f_summary)

        # Write CSV Headers
        labels_writer.writerow(["name.png", "label(yes/no)"])
        summary_writer.writerow(["image_name", "roi_count"])
        ...
        for img_path in image_paths:
            ...
            if not components:
                ...
                summary_writer.writerow([filename, 0])
                continue

            # Stage 2: Priority ROI Planning
            tagged_boxes = generate_rois(components)

            # Stage 3: Extract, Display, Prompt & Save
            num_rois_saved, global_roi_counter = extract_display_and_save_rois(
                img_path, tagged_boxes, global_roi_counter, labels_writer
            )

            summary_writer.writerow([filename, num_rois_saved])
            print(f"[FAIL] {filename} -> Flagged Components: {len(components)} | ROIs Processed: {num_rois_saved}")

    print("\nProcessing complete. Outputs saved:")
    print(f"  └─ ROIs Directory: {OUTPUT_ROIS_FOLDER}/")
    print(f"  └─ Labels CSV: {LABELS_CSV_PATH}")
    print(f"  └─ Frame Summary CSV: {SUMMARY_CSV_PATH}")