...
def render_ui(img_bgr, mask, overlay, metrics_dict, user_state, dashboard_height=None):
    ...
    label_map = {
        "C": ("Correct Detect", (0, 255, 0)),
        "M": ("Missed Defect", (0, 0, 255)),
        "P": ("Pass (Clean)", (255, 255, 255)),
        "F": ("False Alarm", (0, 165, 255))
    }
    
    lbl_key = user_state['label']
    if lbl_key in label_map:
        lbl_str, lbl_color = label_map[lbl_key]
        display_label = f"{lbl_key} ({lbl_str})"
    else:
        display_label = "_"
        lbl_color = (200, 200, 200)

    cv2.putText(dashboard, f"Label       : {display_label}", (col2_x, y_line2), cv2.FONT_HERSHEY_SIMPLEX, T_SCALE, lbl_color, THICKNESS, cv2.LINE_AA)    
    ...
    cv2.putText(dashboard, "C/M/P/F: Correct/Missed/Pass/FalseAlarm", (col3_x, int(85 * scale_factor)), cv2.FONT_HERSHEY_SIMPLEX, T_SCALE * 0.75, (200, 200, 200), max(1, THICKNESS - 1), cv2.LINE_AA)
...
def run_single_video_evaluation(
    ...
    for frame in container.decode(video=0):
        ...
        user_state = {"label": None, "noise": None}
        ...
        while True:
            ...
            if key == ord('q'):
                ...
                user_state['label'] = None
                user_state['noise'] = None
                log_debug("User reset manual inputs.")
            ...
            # Keys 0-3 set Noise Rating (0, 1, 2, 3)
                ...
                if user_state['label'] is None or user_state['noise'] is None:
                ...
                label_text = f"Label: {user_state['label']} | Noise: {user_state['noise']} | Frame: {frame_idx} ({timestamp_sec:.2f}s)"
